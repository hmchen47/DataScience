...
for color in df.color.unique():
    ...
    plt.scatter(df[idx].x, df[idx].y, color=color)
    plt.title('Actual')
...
for clf_name, clf in clfs.items():
    ...
    for color in test.pred_color.unique():
        ...
        plt.scatter(test[idx].x, test[idx].y, color=color)

    # plot the training set as well
    for color in train.color.unique():
        idx = train.color==color
        plt.scatter(train[idx].x, train[idx].y, color=color)
    ...
    plt.plot(train_line_x, train_line_y, 'k--', color="black")
