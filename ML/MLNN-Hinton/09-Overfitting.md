...
+ MAP: Maximum a Posterior
  + proper Bayesian approach
    ...
    + Bayesian have all sort of clever tricks for approximating this horrendous distribution
  + to find the most probable weight vector
  ...
  + easier to work in the log domain: minimize a cost $\implies$ using negative log probabilities
  + math representation
...
+ The Bayesian interpretation of weight decay
  ...
  + $\frac{1}{2\sigma_D^2} \sum_c (y_c - t_c)^2$: assuming that the model makes a Gaussian prediction
  + $\frac{1}{2\sigma_W^2} \sum_i w_i^2$: assuming a Gaussian prior for the weights
