...
+ Key idea
  + a very siple way to learn a feed-forward network
    + make the early layers random and fiexed
    + learn the last layer w/ linear model
    + using the transformed input to predict the target outputs
    + a big random expansion of the input vector is helpful
  + euqivalent idea for RNN
    + fix the input$\to$hidden connections
    + randomly assigned values for th ehidden$\to$hidden connections
    + only learning the hidden$\to$output connections
    + simple learning: asumming linear output units
...
+ Setting the random connections
  + set the hidden$\to$hidden weights
    + the length of he activity vector stays about the same after each iteration
    + allowing the input to echo around the network for a long time
  + using sparse connectivity
    + i.e., setting most of the weights to zero
    + create alots of loosely coupled oscillators
  + carefully choosing the scal eof the input$\to$hidden connections
    + drive the loosely coupled oscillators w/o wiping out the infromation from the past that they already contain
  + fast learning
    + able to trymany different scals for weights and sparasenesses
    + often necessary

+ Simple example
  + input sequence: a real-valued time-varying the specifies the frequence of a sine wave
  + target output sequence: a sine wave w/ the current specified frequency
  + learning method: fit a similar model that takes the states of the hidden units as input and produces a single scalar output

  <div style="margin: 0.5em; display: flex; justify-content: center; align-items: center; flex-flow: row wrap;">
    <a href="http://www.cs.toronto.edu/~hinton/coursera/lecture8/lec8.pdf" ismap target="_blank">
      <img src="img/m08-10.png" style="margin: 0.1em;" alt="Example of an echo state network" title="Example of an echo state network" height=250>
      <img src="img/m08-11.png" style="margin: 0.1em;" alt="Target predicted output after learning" title="Target predicted output after learning" height=100>
    </a>
  </div>
...
+ Beyond echo state networks
  + Godd aspects
    ...
    + modeling of 1-dimensional time-space: unable to compete seriously for high-dimensional data like pre-processed speech
  + Base aspects
    + require more hidden units for a given tak than an RNN that learns the hidden$\to$hidden weights
    + Ilya Stuskever (2012): 
      + if hte weights are initialized using the ESN methods, RNNs cna be trained very effectively
