...
+ Artificial Intelligence and Anti-Probability in 1970's
...
+ Incorporating probability into AI
  ...
  + Graphical models
    ...
    + computing for nodes of the graph, given the states of other nodes
      ...
      + respected the rules of probability: compute the expected values of some nodes in the graph given the observed states of other nodes
  + Belief nets:
...
+ Belief Networks
  + a directed acyclic graph composed of stochastic variables (see diagram)
    + observe any variable in general
  ...
  + Problems to solve:
    + __the inference problem__: infer the states of the unobserved variables
      ...
      + dependent unobserved variable $\implies$ probability distribution likely to be big cumbersome things w/ an exponential number of terms
    + __the learning problem__: adjust the interactions btw variables to make the network more likely to generate the training data
      ...
      + adjusting interactions will involve both
        + deciding which node affected by which other nodes
...
+ Graphical models vs. Neural networks
  + early graphical models
    + using experts to define the graph structure and the conditional probabilities; example: medical experts
      ...
      + typically having a conditional probability table
...
+ Sigmoid Belief Networks
  ...
  + easily to generate an unbiased example at the leaf nodes
    ...
    + learning done; $\to$ the weights decided in the network
    + easily see what the network believes in by generating samples from the model
    + a causal model: following top down sequentially one layer at a time
...
+ Learning rule
  ...
  + maximizing the log probability for each unit
    + the binary state in the sample from the posterior would be generated by the sampled binary states of its parents
    + learning rule: local and simple
    + $p_i$: the probability to turn on node $i$ involves the binary states of the parents
      + making the probability similar to the actual observed binary value of node $i$
    + $\Delta w_{ji}$:
  ...
  + given an assignment of binary states to all the hidden nodes $\implies$ easily to do maximum likelihood learning in typical stochastic way
    ...
    + average the update over a mini-batch of samples
...
+ Explaining away
  ...
  + example: earthquake and truck (see diagram)
    ...
    + observed effect: a house jumps $\gets$ very unlikely to happen unless one of the causes is true
    + one of the causes happened, the 20 cancels the -20 $\implies$ house jumps w/ a probability $p(\text{house jumps}) = 1 / (1 + e^{(20-20)}) = 0.5$
    + each of the causes rather than unlikely but not nearly as unlikely as a house spontaneously jumping
    + house jumping observed, plausible explanantions
      + a truck hits the house
      + an earthquake
    + both explanations w/ a probability about $e^{-10}$ and happening spontaneously about $e^{-20}$
    + assume that an earthquake reduces the probability that house jump because of a truck hits it $\to$ anti-correlation btw the two hidden causes when house jumping observed
...
+ Issue for learning
  ...
  + problem 1: the posterior not factorial because of "explaining away"
    ...
    + not independent in the posterior $\gets$ explaning away
  ...
  + problem 2: posterior depending on the prior as well as the likelihood
    + required to know the weights in higher layers, even w/ approximated posterior
  ...
  + problem 3: required to integrate over all possible configurations in the higher layers to get the prior for the first hidden layer $\to$ hopeless
    + to computer the prior term, require to consider all possible patterns of activity in these higher layers
    + combine them all to compute the prior that the higher layers create for the first hidden layer
    + computing that prior $\to$ very complicated
...
+ Learning methods
  ...
  + sampling from wrong distribution + the maximum likelihood learning rule:
    ...
    + no guarantee on improvement; guarantee = the log probability that the mode will generate the data
