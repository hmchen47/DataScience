...
+ Mechanism under layer-by-layer learning
...
+ An infinite sigmoid belief net equivalent to an RBM
  ...
  + $p(v|h)$ and $p(h|v)$ both defined by $W$
  + top-down pass of the directed net exactly equivalent to letting a RBM settle to equilibrium
  + infinite directed net defines the same distribution as an RBM
...
+ Inference in an infinite belief sigmoid belief net
  + variables in %h_0$
    ...
    + $h_0$ implementing a <span style="color: blue;">complementary prior</span>
...
+ Learning a deep directed network
  + learning rule for a sigmoid belief net (left diagram)
  ...
  + learning w/ all the weights tied
    + exactly equivalent to learning an RBM (middle left diagram)
  ...
  + instead of using CD1 as a shortcut
  + freezing the first layer of weights in both directions
  + learning the remaining weights (still tie together)
...
+ Impact on ignoring the derivatives in higher layers
  ...
  + weight $\nearrow \to$ running more iteration of CD
    ...
    + learning layers of features: not required to be a good approximation to maximum likelihood
