...
+ The partial derivatives of regularized linear regression's cost for $\theta_j$
    ...
    $$\dfrac{\partial J(\theta)}{\partial \theta_0}  = \begin{cases} \dfrac{1}{m} \sum_{i=1}^m (h_\theta(x^{(i)}) - y^{(i)}) x_j^{(i)} & \text{for } j = 0 \\ \left( \dfrac{1}{m} \sum_{i=1}^m (h_\theta(x^{(i)}) - y^{(i)}) x_j^{(i)}  \right) + \dfrac{\lambda}{m} \theta_j & \text{for } j \geq 1$$
