...
+ 3-layer Neural Network
  ...
  <div style="display:flex;justify-content:center;align-items:center;flex-flow:row wrap;">
    <div><a href="https://s3.amazonaws.com/spark-public/ml/exercises/on-demand/machine-learning-ex4.zip">
      <img src="images/e04-01.png" style="margin: 0.1em;" alt="It has 3 layers { an input layer, a hidden layer and an output layer. Recall that our inputs are pixel values of digit images. Since the images are of size 20 x 20, this gives us 400 input layer units (not counting the extra bias unit which always outputs +1)." title="Neural Network Model" width="250">
    </a></div>
  </div>
...
+ The gradient for sigmoid function

  $$g^\prime(z) = \dfrac{d}{d z} g(z) = g(z)(1-g(z))$$

+ Verify by calling `sigmoidGradient(z)`
  + $z \;\rightarrow \pm \infty \implies g(z) = 0$
  + $z = 0 \implies g(z) = 0.25$


### 2.2 Random initialization

+ randomly initialized the parameters for symmetry breaking

+ Effective strategy: select values for $\Theta^{(l)}$ uniformly in the range $[-\epsilon_{init}, \epsilon_{init}]$ w/ $\epsilon_init = 0.12$
...
+ Complete `randInitializeWeights.m` to initialize the wights for $\Theta$

+ Modify and fille the following code:
...
<div style="display:flex;justify-content:center;align-items:center;flex-flow:row wrap;">
  <div><a href="url">
    <img src="images/e04-02.png" style="margin: 0.1em;" alt="The diagram shows the strategy to calculate the back-propagation updates for neural network." title="Backpropagation Updates" width="350">
  </a></div>
</div>
...
+ implement steps 1 to 4 in a loop that processes one example at a time
  + implement a for-loop `for i=1:m`
  + place steps4 1-4 inside the for-loop, with the $t$-th iteration performing the calculation on the $t$-th training example $(x^{(t)}, y^{(t)}$)
...
+ Octave Tip
  + implement backpropagation algorithm only after complete the feedforward and cost functions
  + `size()` useful to print the sizes of the variables

+ The gradient check to increase the confidence of the implementation



### 2.4 Gradient checking

+ Objective: minimizing the cost function $J(\Theta)$
...
+ Gradient checking
  ...
  + assuming $\epsilon = 10^{-4} \implies$ the left-side and right-hand sides of the above will agree at least 4 significant digits (and often many more)

+ implement the function to compute the numerical gradient in `computeNumericalGradient.m`, not required to modify the file

+ execute `checkNNGradients.m` via `ex4.m` to create a small neural and dataset used for checking the gradients

+ if backpropagation implementation correctly, the relative difference $<\; 10^{-9}$

+ Practical Tip
  + efficient: using a small neural network with a relative small number of input units and hidden units lead to small number of parameters
  + each dimension of $\theta$ requires two evaluations of the cost function $\implies$ expensive
  + `checkNNGradients`: create a small random model and dataset which is used with `computeNumericalGradient` ffor gradient checking
  + turn off gradient checking once confident the implementation before running learning algorithm

+ Practical Tip
  + gradient checking works for any function when computing the cost and the gradient
  + `computeNumericalGradient` applied to other implementations, including logistic regression's cost function
...
+ NB. should not be regularizing the first column of $\Theta^{(l)}$ used for the bias term

+ The parameters $\Theta^{(l)}_{ij}\;, i = 1, \ldots, s_{l+1}, \; j = 1, \ldots, s_l$

  $$\Theta^{(l)} = \begin{bmatrix} \Theta^{(l)}_{1,0} & \Theta^{(l)}_{1,1} & \cdots & \Theta^{(l)}_{1,s_l} \\ \Theta^{(l)}_{1,0} & \Theta^{(l)}_{2,1} & \cdots & \Theta^{(l)}_{2,s_l} \\ \vdots & \vdots & \ddots & \vdots \\ \Theta^{(l)}_{s_{l+1},0} & \Theta^{(l)}_{1,1} & \cdots & \Theta^{(l)}_{1,s_l}  \end{bmatrix}$$

+ Octave index: starting from 1 (for both $i$ and $j$), thus `Theta(2, 1) actually corresponds to $\Theta^{(l)}_{2,0}$

+ Modify code to compute `grad` in `nnCostFunction` to account for regularization

+ `ex4.m` will run gradient checking and expect the relative difference $< 10^{-9}$


### 2.6 Learning parameters using `fmincg`

+ `ex4.m` will use `fminunc` to learn a good set of parameters

+ `ex4.m` will report the training accuracy and expect $95.3\%\pm 1\%$ due to random initialization

+ Try more iterations (> 400) and different $\lambda$ to check the accuracy

## 3. Visualizing the hidden layer

+ understanding what neural network is learning by visualizing what the representations captured by the hidden units

+ Visualize what it computes is to find an input x that will cause it to activate 

+ With the 20 by 20 pixel image 
  + the $i$-th row of $\Theta^{(1)}$ is a 401-dim vector that represents the parameter of the $i$-th hidden unit
  + discard bias term, the 400-dim vecor represents the wights from each input pixel to the hidden unit
  + reshape the 400-dim vector into 20 by 20 image and display it

### 3.1 Optional exercise

+ try out different learning settings to ovserve their performance
  + regularization parameter: lambda
  + number of training steps: MaxIter option in `fminunc`

+ Neural network
  + powerful models
  + able to form highly complex decision boundaries
  + "overfit" highly possibly
...
### [ex4 tutorial for nnCostFunction and backpropagation](https://www.coursera.org/learn/machine-learning/discussions/all/threads/a8Kce_WxEeS16yIACyoj1Q)
