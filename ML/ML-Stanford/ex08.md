...
+ complete the code in `cofiCostFunc.m` to compute the cost fucntion and gradient for collaborative filtering
...
+ Implementation Tip
  ...
  + After vectorized he computations of the derivatives wrt $x^{(i)}$, use a similar method to vectorize the derivatives wrt $\that^{(j)}$

+ Vectorized Collaborative filtering Gradients
